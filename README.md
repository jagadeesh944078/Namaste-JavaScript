...
- 1.callback hell(callback inside callback inside callback inside and this code is unreadable and unmaintainable and this structure is also known as pyramid of doom)
- 2.inversion of control(we loose control over the program when we use the callback. here we have given control of the function to the some another function. now we dont know whether that function ever execute callback or not) this is invert of control another big issue in callback
