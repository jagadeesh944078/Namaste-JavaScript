...
# let us now see what happens in second phase

- second phase is code execution phase
- in this phase this 2 value over here of n is now being placed actually in this placeholder or the identifier which is n now this 2 value in the second phase allocated in the place of undefined
- after finishing line one it goes to line second sees that we nothing to do here from 2 to 5 line then it moves to line 6
...
- rememeber whenever function is invoked a new execution context is created so this execution context put inside the call stack so **let me just denote it by E1** which is execution context 1 once we are done with executing this function we return the **ans** so what happened now this **E1 is popped out of the stack** and control goes back to the global execution context where it left
- so control goes back to the line number 6 okay? got it?
- and then we move on to line number 7 where the new function was invoked and new execution will be created similarly E2 will move up to the stack once whole function was executed then E2 will also move out of stack and control goes back to global execution context
